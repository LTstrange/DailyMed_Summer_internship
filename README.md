...
###### 1、运行web_crawler_4_setIDs.py 下载所有setIDs。

若程序崩溃或长时间无打印信息，直接停止程序或关闭命令行，并重新运行。程序支持断点恢复。

在看到web_crawler_4_setID.py程序，打印出“所有setID已经下载完毕。下一步，请运行 'read_setIDs.py' 文件。”字样后。说明程序运行完毕，可进行下一步。

###### 2、运行read_setIDs.py整理已下载的setIDs。

程序将在几秒内完成对所有已下载的setID的整理，并在程序文件目录生成setIDs.txt文件。

并打印出“已经整理出 x 个setID。”和“所有setID已经整理完毕。下一步，请运行'web_crawler_4_spls.py'文件。”字样后。说明程序运行成功，并可进行下一步。

###### 3、运行web_crawler_4_spls.py下载SPL格式说明书。

程序会根据setIDs.txt下载SPL说明书。部分链接或setID可能失效，程序会打印出报错及未成功的setID编号，并继续运行。

只需要等程序运行完毕后，再次运行本程序，程序会再次下载未下载的SPL。

可能存在始终无法下载的数据，需要通过报错后面的setID编号，在DailyMed网站搜索，查看具体原因。
